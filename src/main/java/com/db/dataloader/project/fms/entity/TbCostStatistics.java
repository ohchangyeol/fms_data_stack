package com.db.dataloader.project.fms.entity;

import jakarta.persistence.*;
import java.sql.*;

import com.db.dataloader.project.fms.entity.pk.TbCostStatisticsId;

@IdClass(TbCostStatisticsId.class)
@Entity
@Table( schema = "INITUFMSNEW", name = "tb_cost_statistics")
public class TbCostStatistics {
    @Id private String cost_statistics_classification_cd;
    @Id private Timestamp occurrence_dt;
    @Id private long vehicle_id;
    private long cost_incurred;
    private String is_active;
    private Timestamp created_at;
    private String register_id;
}
