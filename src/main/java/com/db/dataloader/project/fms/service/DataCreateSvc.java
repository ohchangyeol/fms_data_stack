package com.db.dataloader.project.fms.service;
...
import com.db.dataloader.project.fms.resource.TableDefinitionEnum;
...
public class DataCreateSvc {
    ...
    public void dataCreateAll() {
        ...
        for (TableCountResult tableCountResult :tableCountResults){
            if(!tableCountResult.isMatchStatus()){
                ...
                TableDefinitionEnum tableInfo = TableDefinitionEnum.findByNameAndSchema(tableCountResult.getTableSchema(), tableCountResult.getTableName());
                        ...
                        for (int i = 0; i < tablePartitionDetail.getTableRows(); i++) {
                            ...
                            for (TableColumnDetail detail : tableColumns) {
                                Object value = "NO".equals(detail.getIsNullable()) ?
                                        FmsUtils.setFieldValue(detail, startIdx, ts) : FmsUtils.setFixValue(detail, null);
                                colunms.add(value);
    ...
    private List<TablePartitionDetail> createPartitionDifferenceCount(TableDefinitionEnum table , boolean isPartition , int differenceCount)  {
    ...
    private List<TablePartitionDetail> getFinishPartitionCountList(TableDefinitionEnum table) {
